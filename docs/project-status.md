...
| 本轮外部行为 | 本轮只改 Worker 心跳的续期延迟预算与其测试、文档；不调用真实 IdP 或模型、不做真实业务动作或生产部署。修复前该用例本机 20 次失败 11 次，修复后 20 次全部通过；真实 PostgreSQL 全量 504 项通过。远端 CI run [34957131285](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34957131285) 为 `success`（`postgres:17` 上 `504 passed`），`main` 已恢复绿色。连接池、Broker 与压测仍未做 |
...
| D-01-06 | DONE | 新增工作台单工单访问管理：`web/src/grants.ts`（闭集镜像、委派上限、有效期与 revision 校验、状态派生）、`GrantPanel.tsx`（列出/授予/替换/撤销）与 `App.tsx` 接线（服务端拒绝时静默隐藏面板）；前端 38 项单测、`tsc --noEmit` 与生产构建通过，并用真实浏览器对真实 API + 真实 PostgreSQL 走完授予→撤销→替换全流程（8 项断言，含 SSE 实时事件）；同时修掉时间线折叠缺陷（`reduce` 把下标当 `cap`，新事件到达会清空时间线）；租户级授权总览与跨工单发现仍属 D-01 其余部分 |
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 A2-02 修正。D-01-06 的交付提交 `01ed4f6` 已在 `origin/main`，但其 CI run [34955596978](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34955596978) 为 `failure`（`1 failed, 501 passed`），失败项是心跳用例，与该提交只改 web/docs 的范围无关；本轮据此先修心跳再推送：提交 `230e518` 的 CI run [34957131285](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34957131285) 为 `success`（`postgres:17` 上 `504 passed`），`main` 已恢复绿色。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；项目许可证仍待用户确定。
