...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：按路线图持续把项目推向企业级，并在本轮明确授权提交/推送到 GitHub。本轮做 D-01-07：让持 `grant:read` 的访问管理员看得见租户内自己未参与的工单，否则 D-01-06 的移交面板只能作用于管理员本来就参与处理的工单；不调用真实 IdP、不连接生产数据库、不做外部业务动作 |
| 当前任务 | D-01-07 访问管理发现（控制面）：新增 `GET /v1/administration/cases`、清单投影闭集 `ADMINISTRATION_PERMISSIONS`、授权台账的调用者字段 `delegable`/`can_administer`，以及工作台的双列表拆分 |
...
| 下一项代码候选 | D-04：按已测量的建连开销评估连接池（含 `PostgresEventTail` 每轮建连）；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览；A3-04：Harness 等待与审批分支纳入评测；B-02-03：供应商回执核对；C-02：真实沙箱后端 |
| 活跃实现任务 | 本轮改 `aftercare_agent/api/app.py`（新路由 `GET /v1/administration/cases`、`_administration_summary()`、`CaseGrantListResponse` 新增 `delegable`/`can_administer`）、`aftercare_agent/auth/grants.py`（新增 `ADMINISTRATION_PERMISSIONS`）、`aftercare_agent/persistence/admission.py`（`list_for_tenant()` 增加 `case_ids` 上界并改写 docstring 的两类合法调用者）、`tests/test_operator_grants_integration.py`（新增 5 项集成测试）与 `web/`（`cases.ts`、`CaseList`/`CaseDetailPane`/`GrantPanel`/`App` 的双列表拆分）；连接池与租户级授权总览仍未实现 |
| 本轮外部行为 | 本轮只改控制面读取与工作台；不调用真实 IdP 或模型、不做真实业务动作或生产部署。真实 PostgreSQL 16.13 重建库后全量 `509 passed`，离线 `407 passed, 102 skipped`，前端 43 项单测 + `tsc --noEmit` + 生产构建通过；提交 `f04cc09`（交付）与 `95384c3`（浏览器验证抓出的两个工作台缺陷）已推送，CI run [34958850865](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34958850865) 与 [34959917168](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34959917168) 均为 `success`（`postgres:17` 上各 `509 passed`）；另有两轮真实浏览器冒烟（真实 Bearer 管理员 10 项 + 合成身份回归 9 项）。连接池、Broker 与压测仍未做 |
...
本轮提交范围为 D-01-07 访问管理发现：`aftercare_agent/api/app.py`（新路由 `GET /v1/administration/cases`、`_administration_summary()` 清单投影、`CaseGrantListResponse` 新增 `delegable`/`can_administer`）、`aftercare_agent/auth/grants.py`（新增 `ADMINISTRATION_PERMISSIONS` 闭集，与 `GRANTABLE_CASE_PERMISSIONS` 分开维护并写明理由）、`aftercare_agent/persistence/admission.py`（`CaseRepository.list_for_tenant()` 接受只收窄的 `case_ids` 上界，docstring 明确两类合法调用者）、`tests/test_operator_grants_integration.py`（新增 5 项集成测试与两个辅助函数）、`web/src/cases.ts`（新增，队列/清单的行拆分）、`web/src/api.ts`（抽出 `caseQueryParams()`、新增 `listAdministrableCases()`）、`web/src/types.ts`、`web/src/components/{CaseList,CaseDetailPane,GrantPanel}.tsx`、`web/src/App.tsx`（双列表与选中项修复）、`web/src/styles.css`、`web/src/cases.test.ts`（新增）与 `web/src/api.test.ts`（新增 1 项）；文档同步 `docs/decisions/0005-access-administration-discovery.md`（新增 ADR）、`docs/api-auth.md`、`docs/engineering-plan.md`、`web/README.md`、本文件。第二个提交 `95384c3` 是浏览器验证抓出的两个工作台缺陷：`web/src/App.tsx`（仅可管理的行不再订阅事件流——事件是内容路由，此前每次选中都会发一个必然 403 的请求）与 `web/src/components/CaseList.tsx`（清单区块不再复用队列的空态文案，改为描述“已加载行都在队列里”）。未包含 .venv、node_modules、dist、缓存、临时数据或 EGM 仓库改动。
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 D-01-07。交付提交 `f04cc09` 与修复提交 `95384c3` 已推送到 `origin/main`，CI run [34958850865](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34958850865) 与 [34959917168](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34959917168) 均为 `success`（`postgres:17` 上各 `509 passed`）。进入本轮时 `main` 是绿色的（上一轮 run 34957312393 为 `success`），本轮不需要先修回归。本文件自身的台账提交在最后推送，其 CI 结果按当时实际 run 记录（若尚未推送，则以 Git 日志为准）。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；项目许可证仍待用户确定。
...
当前推进 D 生产准入，并按已测量的建连开销把 D-04 连接池提到候选首位：验签（D-01-01/02）、数据库 CaseGrant（D-01-03）、撤销判定（D-01-04）、授权管理面（D-01-05）、工作台单工单访问管理（D-01-06）与租户工单发现（D-01-07）已实现，D-01 剩下真实 IdP 演练与权限映射、RLS 与租户级授权总览（“谁对这个租户的哪些工单有权限”，D-01-07 只解决“有哪些工单”，不列出谁有权限）；A2-02 的心跳续期延迟预算已修正。D-04 现在不再只是“按瓶颈评估”的占位：`Database.transaction()` 每个工作单元建一次连接（本机实测 p50 115 ms），同一套件在本机 71.69 s、在 CI Linux 12.4 s，`PostgresEventTail` 每轮轮询同样付一次建连，300 ms 尾部窗口在慢建连主机上会漏事件——这些都可复现。仍未实现：固定调查 EGM schema、真实模型接线、真实沙箱后端、完整业务 Harness 与生产连接器。

尚待决定但不阻塞离线骨架：真实模型 ID/预算、商家渠道和身份提供者、沙箱/对象存储后端与地域、RPO/RTO 和生产负载目标。每项的决策阶段已列在技术栈和执行计划中。无业务凭证不阻塞 Fake 流程；真实接入缺授权时必须停止该分支。

关键风险：本轮包依赖升级尚未重跑 EGM PostgreSQL 全量验收，A1 必须补足；新旧路线图的阶段名称必须一致；不能把 fencing 延后为性能优化；A 阶段通知不得真实外发；同进程不等于同事务；未提交图稿和其他仓库改动不属于本任务；`grant:admin` 在真实 IdP 里必须只发给管理角色，否则等于交出 ACL 写入权。D-01-07 之后新增一条同源风险：`grant:read` 持有者能看到本租户**全部**工单编号、订单号与状态，它不再是“无害的只读 scope”，真实 IdP 映射时必须和管理角色同等对待；同时“控制面清单 ≠ 数据面授权”这条边界不能在下一次迭代里为了“顺手少一次请求”而被合进 `list_accessible`。
