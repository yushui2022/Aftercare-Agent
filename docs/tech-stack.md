...
aftercare_agent/
  ...
  ops/                        备份、恢复演练、保留删除、恢复后核对（运维工具，不在请求路径）
web/                          A3 工作台
...
当前尚待确定：后续新增依赖与镜像补丁版本（A1/A3）、真实模型 ID 与预算（A3）、客户身份提供者及通知渠道（试点前）、沙箱与对象存储后端/地域（C）、备份与恢复的目标 RPO/RTO 和负载目标（D）。D-02 已给出备份、演练与核对的工具和口径（[ADR-0008](decisions/0008-backup-and-restore-drills.md)），但目标值必须在具体部署上按节奏与演练确定，本机数字不能替代。这些是有阶段归属的开放项，不是默许使用任意默认值。
