...
| 当前任务 | D-02 备份与恢复演练（第三刀：WAL 归档检查）：新增 `aftercare_agent/ops/wal_archive.py`（`Segment`/`adjacent()`/`Gap`/`ArchiveInventory`/`ArchiverState`/`ArchiveBudget`/`Coverage`/`ArchiveReport`、`evaluate_archive()`、`report_archive_status()` 发布 `aftercare.backup.wal_*`、`segment_holding()`/`parse_segment_size()`/`parse_lsn()`），`backup.py` 给 `DatabaseSnapshot`/`BackupManifest` 加 `wal_lsn`（`manifest_version` 升到 2，v1 仍可读但覆盖问题答不了）并新增 `wal` 子命令，`freshness.py` 的 `_age_text()` 公开为 `age_text()` 供两处复用；决策见 [ADR-0010](decisions/0010-wal-archive-checks.md)，操作步骤见 [备份与恢复](operations/backup-restore.md) 第 7 节 |
