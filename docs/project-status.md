...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：按路线图持续把项目推向企业级，并在本轮明确授权提交/推送到 GitHub。本轮做 D-02 的第一刀：把“备份成功”的判据从作业退出码换成一次真实恢复演练——`aftercare-backup` 产出 dump、描述它的清单和演练报告，`drill` 在临时副本上还原并逐表比对，`reconcile` 列出恢复点无法描述的 Action；不连接生产数据库、不做真实业务动作、不发布包、不做异地或加密副本 |
| 当前任务 | D-02 备份与恢复演练：`aftercare_agent/ops/` 新增 `tooling.py`（工具解析与 major 校验、DSN/子进程环境、库名白名单）、`backup.py`（`aftercare-backup`：`create`/`verify`/`drill`/`retention`/`reconcile`，退出码 0/1/2）与 `reconcile.py`（恢复后核对分类，含默认 60 s 安全边界）；`persistence/db.py` 抽出 `known_migrations()`/`latest_schema_version()` 作为迁移清单的唯一定义；决策见 [ADR-0008](decisions/0008-backup-and-restore-drills.md)，操作步骤见 [备份与恢复](operations/backup-restore.md)，本机演练报告见 `docs/operations/2026-09-15-restore-drill.json` |
| 当前阶段 | A1-03 DONE；A1-04 最小 Worker/Compose 已有；A2-01 Wait/Inbox/Outbox 与 gap buffer 基础已落地；A2-02 心跳/常驻轮询已落地并完成续期延迟预算修正；B-01 Action Ledger 最小闭环已落地；A3-03-a/b 工作台与 SSE 已完成；D-01-04 撤销判定、D-01-05 授权管理面、D-01-06 工作台访问管理与 D-01-07 租户工单发现已落地；D-04 有界连接池（ADR-0006）、D-03 池指标/容量 sweep（ADR-0007）与 D-02 备份/清单/恢复演练（ADR-0008）已落地 |
| 下一项代码候选 | D-02 剩余：WAL 归档与 PITR、对象存储与异地副本、备份加密与密钥托管、定期演练的自动化与告警、按部署目标确定 RPO/RTO、多租户选择性恢复；D-03 剩余：在容器与 CI 复跑同一条 sweep、把面向业务的队列年龄（Run 队列等待时间）接进观测、在有目标工作负载的部署上定标；C-03：给已接好的 Tracer/Metrics 配导出器与留存；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览；A3-04：Harness 等待与审批分支纳入评测；B-02-03：供应商回执核对；C-02：真实沙箱后端 |
| 活跃实现任务 | 本轮新增 `aftercare_agent/ops/tooling.py`、`aftercare_agent/ops/backup.py`（一次 `REPEATABLE READ` 事务里 `pg_export_snapshot()` 同时喂给清单读取与 `pg_dump --snapshot`，因而不一致读不会进清单）与 `aftercare_agent/ops/reconcile.py`；改 `aftercare_agent/persistence/db.py`（`known_migrations()`/`latest_schema_version()`）、`pyproject.toml`（`aftercare-backup` 入口）、`.github/workflows/ci.yml`（把 `postgresql-client-17` 装好并放进 `PATH`，恢复演练才有匹配的客户端）与 `deploy/compose/README.md`（备份是运维任务，Compose 不提供备份服务）；测试新增 `tests/ops/test_backup.py` 25 项、`tests/ops/test_reconcile.py` 7 项与 `tests/persistence/test_backup_restore.py` 7 项（需要 `DATABASE_URL` 与 major 匹配的客户端，否则带原因跳过）；文档新增 ADR-0008、`docs/operations/backup-restore.md` 与本机演练报告；Broker、暖池、ACP、长期记忆与额外供应商仍未启用 |
| 本轮外部行为 | 本轮让一条真实的备份→恢复→核对进程链路可跑，并在 CI 的 `postgres:17` 上真跑了一次恢复演练；不连接生产数据库、不做真实业务动作或生产部署、不上传任何副本。本机（临时 PostgreSQL 16.13）重建库后全量 `580 passed`（55.88 s），离线全量 `459 passed, 121 skipped`，`ruff format --check`/`ruff check`/严格 `mypy`（126 文件）与 `docker compose config --quiet` 通过；前端未改也未运行。提交与 CI 结果见第 5、6 节 |
...
这是本轮改动开始时核验的源码基线。D-04 起 `Database` 默认持有每进程一个有界 psycopg 连接池（ADR-0006）；D-03 起 `Database.stats()` 是池健康的唯一读取口，每个进程默认每 10 s 发布一次 `aftercare.db.pool.*`（ADR-0007）；D-02 起 `db.known_migrations()` 是迁移清单的唯一定义，`migrate()` 与备份清单都读它，因此“清单里的迁移摘要”和“实际建表的那份迁移”不可能各说一套（ADR-0008）。历史验证条目保留原日期和当时边界；本轮交付后的 HEAD 和远端状态以 Git 日志为准。
...
本轮提交范围为 D-02 备份与恢复演练：新增 `aftercare_agent/ops/__init__.py` 与 `aftercare_agent/ops/tooling.py`（`OpsError`/`MissingToolError`/`IncompatibleToolError`、`OpsModel`（`extra=forbid`）、`Tool`/`CommandResult`/`CommandRunner` Protocol/`SubprocessRunner`、`parse_major()`、写文件前校验 major 的 `resolve_tool()`、`validate_database_name()`、`connection_parameters()`/`dsn_with_database()`/`conninfo_without_password()`/`child_environment()`，密码只经 `PGPASSWORD`）、`aftercare_agent/ops/backup.py`（`MigrationRecord`/`TableRows`/`DatabaseSnapshot`/`BackupManifest`（`manifest_version=1`）、`read_migrations`/`read_row_counts`/`read_schema`/`read_snapshot`、`finalize_backup`、`create_backup`、`Finding`/`Verification`/`verify_backup`、`RestoreDrill`/`run_restore_drill`、`RetentionPolicy`/`RetentionDecision`/`RetentionPlan`/`plan_retention`/`apply_retention`、`build_parser`/`main`）与 `aftercare_agent/ops/reconcile.py`（`ReconciliationReason`、`ReconciliationEntry`/`ReconciliationReport`、`classify`/`build_reconciliation`，默认 60 s 安全边界、默认上限 200/最大 1000）；改 `aftercare_agent/persistence/db.py`（抽出 `known_migrations()`/`latest_schema_version()`，`migrate()` 改为复用同一份定义，行为不变）、`pyproject.toml`（`aftercare-backup` 入口）、`.github/workflows/ci.yml`（装 `postgresql-client-17` 并把 `/usr/lib/postgresql/17/bin` 写进 `GITHUB_PATH`）、`deploy/compose/README.md`（备份是运维任务，Compose 不提供备份服务）；测试新增 `tests/ops/__init__.py`、`tests/ops/test_backup.py`（25 项）、`tests/ops/test_reconcile.py`（7 项）与 `tests/persistence/test_backup_restore.py`（7 项，需 `DATABASE_URL` 与 major 匹配的客户端，否则带原因跳过）；文档新增 `docs/decisions/0008-backup-and-restore-drills.md`（10 条决策、本机实测表与后果）、`docs/operations/backup-restore.md` 与 `docs/operations/2026-09-15-restore-drill.json`，同步 `docs/development.md`、`docs/persistence.md`、`docs/tech-stack.md`、`docs/engineering-plan.md`、`ROADMAP.md` 与本文件。未包含 .venv、缓存、留在 `G:\DevCache\Temp\aftercare-agent` 的演练产物或 EGM 仓库改动。
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 D-02（备份与恢复演练）。交付提交 `1afe587`（实现）与 `1e9d9ef`（CI：把 PostgreSQL 17 客户端放进 `PATH`，第一次推送时演练用例因客户端 16 而跳过）已推送到 `origin/main`；两次 CI run [34967406421](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34967406421) 与 [34967559546](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34967559546) 的 `Static checks` 与 `Test with PostgreSQL` 全部为 `success`，后者在 `postgres:17` 上 `580 passed, 98 warnings in 13.15s`、零跳过。本文件自身的台账提交 `0947b7c` 也已推送，CI run [34968364743](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34968364743) 同为 `success`（`580 passed, 98 warnings in 20.27s`）。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；项目许可证仍待用户确定。
