...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：按路线图持续把项目推向企业级，并在本轮明确授权提交/推送到 GitHub。本轮做 D-03 的第一刀：把池指标接进观测（`Database.stats()`、单一发布者 `PoolStatsSampler`、`aftercare.db.pool.*`），并用新的 `aftercare-capacity` 在真实 PostgreSQL 上按 steady/burst/wake 三种形状扫描 `max_size`，据此判断默认值该不该改；不调用真实 IdP 或模型、不连接生产数据库、不做真实业务动作、不发布包 |
| 当前任务 | D-03 池容量定标与池指标：`Database.stats()` 返回只含整数的 `PoolStats` 快照；`aftercare_agent/observability.py` 新增 Metrics 接缝（`InMemoryMetrics`、`LoggingMetrics`、可选 OTel bridge）；`aftercare_agent/persistence/pool_metrics.py` 用 `PoolStatsSampler` 每 10 s 发布 gauge/counter（`AFTERCARE_POOL_METRICS=0` 关闭）；`aftercare_agent/capacity.py`（`aftercare-capacity`）做实测 sweep 并给出达标尺寸；决策与结论见 [ADR-0007](decisions/0007-pool-metrics-and-capacity.md) 与 [容量报告](capacity/README.md) |
| 当前阶段 | A1-03 DONE；A1-04 最小 Worker/Compose 已有；A2-01 Wait/Inbox/Outbox 与 gap buffer 基础已落地；A2-02 心跳/常驻轮询已落地并完成续期延迟预算修正；B-01 Action Ledger 最小闭环已落地；A3-03-a/b 工作台与 SSE 已完成；D-01-04 撤销判定、D-01-05 授权管理面、D-01-06 工作台访问管理与 D-01-07 租户工单发现已落地；D-04 有界连接池（ADR-0006）与 D-03 池指标/容量 sweep（ADR-0007）已落地 |
| 下一项代码候选 | D-03 剩余：在容器与 CI 复跑同一条 sweep、把面向业务的队列年龄（Run 队列等待时间）接进观测、在有目标工作负载的部署上定标；C-03：给已接好的 Tracer/Metrics 配导出器与留存；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览；D-02：备份恢复演练与 RPO/RTO；A3-04：Harness 等待与审批分支纳入评测；B-02-03：供应商回执核对；C-02：真实沙箱后端 |
| 活跃实现任务 | 本轮新增 `aftercare_agent/capacity.py`（steady/burst/wake sweep、p95 预算判据、`--service-time-ms`/`--min-size`、JSON 报告）与 `aftercare_agent/persistence/pool_metrics.py`（指标映射与采样器）；改 `aftercare_agent/persistence/db.py`（`PoolStats`、`Database.stats()`、`startup(timeout=...)`）、`aftercare_agent/observability.py`（Metrics 接缝与两个可选 bridge）、`aftercare_agent/api/app.py` 与 `aftercare_agent/runtime/worker_cli.py`（采样器生命周期）、`pyproject.toml`（`aftercare-capacity` 入口）、`deploy/compose/`（指标环境变量透传）；测试新增 4 个文件 24 项；文档新增 ADR-0007 与 `docs/capacity/`（含三份报告）；Broker、暖池、ACP、长期记忆与额外供应商仍未启用 |
| 本轮外部行为 | 本轮只改观测与容量口径，并让 API/Worker 每 10 s 往进程日志写一行池指标；不调用真实 IdP 或模型、不做真实业务动作或生产部署。真实 PostgreSQL 16.13 重建库后全量 `541 passed`（34.06 s），离线全量 `427 passed, 114 skipped`，`ruff format --check`/`ruff check`/严格 `mypy`（118 文件）与 `docker compose config --quiet` 通过；前端未改也未运行。提交与 CI 结果见第 5、6 节 |
...
这是本轮改动开始时核验的源码基线。D-04 起 `Database` 默认持有每进程一个有界 psycopg 连接池（ADR-0006）；D-03 起 `Database.stats()` 是池健康的唯一读取口，每个进程默认每 10 s 发布一次 `aftercare.db.pool.*`（ADR-0007）。历史验证条目保留原日期和当时边界；本轮交付后的 HEAD 和远端状态以 Git 日志为准。
...
尚未存在：完整业务 Harness、支付聚合、真实供应商连接器、沙箱接线、生产调度体系和高吞吐 live broker tail。调查 EGM 适配器和 PostgreSQL 观察账本已建立代码边界，但真实 EGM 调查 schema 尚未固定。当前已有 Fake Harness、合成 Aftercare 纵向切片（含订单/物流/买家观察与来源引用评估）、一次性/常驻 Worker、Wait/Inbox/Outbox publisher、gap buffer、Action Ledger、审批台账/派发门禁、审批 Wait 原子唤醒、跨实例 admission 最小闭环、显式合成身份或静态 JWKS Bearer 的 Review/Approval 控制面 API、CaseGrant 及其授权管理 HTTP（移交/授权/撤销，见 ADR-0004）、操作员工单发现 API、React 工作台、case-scoped SSE replay/tail，以及 D-03 的池指标与容量 sweep。仍不是完整执行服务。真实 IdP 演练与权限映射、RLS、授权管理 UI、生产认证验收、浏览器可访问性、高吞吐 broker、OTel 导出器与跨环境容量结论仍未完成；`aftercare-capacity` 目前只在一台开发机上跑过。不要输出不存在的完整服务启动命令。
...
本轮提交范围为 D-03 池指标与容量定标：`aftercare_agent/persistence/db.py`（`PoolStats` 冻结快照、`Database.stats()`、`startup(timeout=...)` 让预热预算独立于借用预算）、`aftercare_agent/observability.py`（`MetricRecord`/`Metrics`/`InMemoryMetrics`/`LoggingMetrics` 与可选 OTel bridge，并把两个 bridge 的可选导入改成 `importlib` 以避开 mypy 对同一文件第二次可选导入的误报）、`aftercare_agent/persistence/pool_metrics.py`（`report_pool_stats` 与 `PoolStatsSampler`，含 `AFTERCARE_POOL_METRICS*` 解析）、`aftercare_agent/capacity.py`（新增 `aftercare-capacity`：三种并发形状、`--p95-budget-ms`/`--service-time-ms`/`--min-size`/`--json`、退出码 0/1）、`aftercare_agent/api/app.py` 与 `aftercare_agent/runtime/worker_cli.py`（采样器随进程启动与停止）、`pyproject.toml`（新增入口点）、`deploy/compose/`（透传 `AFTERCARE_POOL_METRICS*`）；测试新增 `tests/persistence/test_pool_metrics.py`（7 项）、`tests/test_capacity.py`（10 项）、`tests/persistence/test_pool_capacity.py`（4 项）与 `tests/test_observability.py` 扩充（3 项），并把 `tests/test_token_revocation.py` 的 shutdown 钩子断言更新为含采样器；文档新增 `docs/decisions/0007-pool-metrics-and-capacity.md`、`docs/capacity/`（README 与三份 JSON 报告），同步 `docs/tech-stack.md`、`docs/development.md`、`docs/engineering-plan.md`（D-03/D-04 行）与本文件。未包含 .venv、缓存、留在 `G:\DevCache\Temp\aftercare-agent` 的探针脚本或 EGM 仓库改动。
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 D-03（池指标与容量定标）。交付提交 `e3bedbc` 已推送到 `origin/main`；CI run [34965278041](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34965278041) 的 `Static checks` 与 `Test with PostgreSQL` 全部通过（`postgres:17` 上 `541 passed, 98 warnings in 16.50s`）。本文件自身的台账提交在最后推送。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；项目许可证仍待用户确定。
...
当前推进 D 生产准入。D-04 已落地有界连接池（[ADR-0006](decisions/0006-bounded-connection-pool.md)）：`Database` 默认带池、借用超时 fail-closed、`startup()` 预热、心跳改用 `open()`/`release()`，Broker、暖池、ACP、长期记忆与额外供应商明确保留为候选并各自写下触发条件。验签（D-01-01/02）、数据库 CaseGrant（D-01-03）、撤销判定（D-01-04）、授权管理面（D-01-05）、工作台单工单访问管理（D-01-06）与租户工单发现（D-01-07）已实现，D-01 剩下真实 IdP 演练与权限映射、RLS 与租户级授权总览（“谁对这个租户的哪些工单有权限”，D-01-07 只解决“有哪些工单”）；A2-02 的心跳续期延迟预算已修正。仍未实现：固定调查 EGM schema、真实模型接线、真实沙箱后端、完整业务 Harness 与生产连接器。D-03 已交付定标方法与第一轮实测（[ADR-0007](decisions/0007-pool-metrics-and-capacity.md)、[容量报告](capacity/README.md)）：`aftercare-capacity` 按 steady/burst/wake 扫描尺寸并以 p95 预算判定达标，池指标 `aftercare.db.pool.*` 默认每 10 s 发布到进程日志。本机结论是突发先撞握手与懒增长而不是上限（懒增长时 1–32 没有尺寸达标；预热到 16 后 `max_size=16` 达标且 32 无差别），因此 min 1 / max 8 / 超时 5 s 仍然是**有界默认值**而不是容量结论。下一步优先顺序：在容器与 CI 复跑同一条 sweep、把面向业务的队列年龄（Run 队列等待时间）接进观测，然后才是 C-03 导出器、D-01 剩余（真实 IdP 演练与权限映射、RLS、租户级授权总览）与 D-02 备份恢复演练。
