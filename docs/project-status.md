...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：按路线图持续把项目推向企业级，并在本轮明确授权提交/推送到 GitHub。本轮做 D-02 的第二刀，把“备份新鲜不新鲜、演练到底做过没有”变成可判定的状态：`aftercare-backup` 新增 `status` 子命令（RPO 与演练间隔两个预算、退出码 0/1/2、`--emit-metrics` 走既有 Metrics 接缝），`drill` 无论成功还是失败都在 dump 旁写 `<name>.drill.json` 记录，保留策略连同记录一起删；不连接生产数据库、不做真实业务动作、不发布包、不做异地或加密副本，仓库也不内置调度器 |
| 当前任务 | D-02 备份与恢复演练（第二刀：演练记录与新鲜度判定）：`aftercare_agent/ops/` 新增 `drill_records.py`（`DrillCheck`/`DrillRecord`、`record_version=1`、`OpsModel` 即 `extra=forbid`、`DRILL_SUFFIX`、`MAX_DETAIL=400` 截断、`drill_record_path()`/`write_drill_record()`/`load_drill_records()`）与 `freshness.py`（`ManifestLike`/`StatusBudget`/`Verdict`/`BackupStatus`、`evaluate_status()`、`report_backup_status()` 发布 `aftercare.backup.*`），`tooling.py` 收下 `BACKUP_NAME_PATTERN`/`BackupName`/`validate_backup_name()` 成为唯一的名字边界（演练记录文件名共用同一份定义），`backup.py` 新增 `status` 子命令并让 `drill` 成功失败都留记录；决策见 [ADR-0009](decisions/0009-backup-freshness-and-drill-records.md)，操作步骤见 [备份与恢复](operations/backup-restore.md) 第 6 节 |
| 当前阶段 | A1-03 DONE；A1-04 最小 Worker/Compose 已有；A2-01 Wait/Inbox/Outbox 与 gap buffer 基础已落地；A2-02 心跳/常驻轮询已落地并完成续期延迟预算修正；B-01 Action Ledger 最小闭环已落地；A3-03-a/b 工作台与 SSE 已完成；D-01-04 撤销判定、D-01-05 授权管理面、D-01-06 工作台访问管理与 D-01-07 租户工单发现已落地；D-04 有界连接池（ADR-0006）、D-03 池指标/容量 sweep（ADR-0007）与 D-02 备份/清单/恢复演练（ADR-0008）、D-02 第二刀的演练记录与新鲜度判定（ADR-0009）已落地 |
| 下一项代码候选 | D-02 剩余：WAL 归档与 PITR、对象存储与异地副本、备份加密与密钥托管、按部署目标确定 RPO/RTO 的**数值**、把定期演练接进目标环境的调度与告警、演练失败后的自动升级路径、多租户选择性恢复；D-03 剩余：在容器与 CI 复跑同一条 sweep、把面向业务的队列年龄（Run 队列等待时间）接进观测、在有目标工作负载的部署上定标；C-03：给已接好的 Tracer/Metrics 配导出器与留存；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览；A3-04：Harness 等待与审批分支纳入评测；B-02-03：供应商回执核对；C-02：真实沙箱后端 |
| 活跃实现任务 | 本轮新增 `aftercare_agent/ops/drill_records.py`（演练记录的读写与边界）与 `aftercare_agent/ops/freshness.py`（新鲜度判定与 `aftercare.backup.*` 指标）；把 `BACKUP_NAME_PATTERN`/`BackupName`/`validate_backup_name()` 从 `backup.py` 移入 `aftercare_agent/ops/tooling.py`；改 `aftercare_agent/ops/backup.py`（新增 `status` 子命令与 `_budget_seconds()`，`drill` 成功与失败都写记录，`apply_retention` 同时删除 `.drill.json`）；测试新增 `tests/ops/test_freshness.py` 17 项、`tests/persistence/test_backup_restore.py` 增 2 项（共 9 项），`tests/ops/test_backup.py` 的 import 改到 `tooling`；文档新增 ADR-0009 并更新 `docs/operations/backup-restore.md`、`docs/development.md`、`docs/tech-stack.md`、`docs/engineering-plan.md`、`ROADMAP.md`、`deploy/compose/README.md` 与本文件；Broker、暖池、ACP、长期记忆与额外供应商仍未启用 |
| 本轮外部行为 | 本轮让“备份是否新鲜、演练是否做过”可以被调度按退出码判定，并在记录不可读时明确失败而不是跳过；不连接生产数据库、不做真实业务动作或生产部署、不上传任何副本、不安装调度器。本机（临时 PostgreSQL 16.13）重建库后全量 `599 passed`（61.23 s），离线全量 `476 passed, 123 skipped`，`ruff format --check`/`ruff check`/严格 `mypy`（129 文件）与 `docker compose config --quiet` 通过；远端 CI run [34970075358](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34970075358) 在 `postgres:17` 上 `599 passed, 98 warnings in 21.12s`、零跳过。前端未改也未运行。提交与 CI 结果见第 5、6 节 |
...
| D-02 | IN PROGRESS | 交付备份与恢复演练（[ADR-0008](decisions/0008-backup-and-restore-drills.md)、[运维手册](operations/backup-restore.md)）：`aftercare-backup` 的 `create` 在同一个 `REPEATABLE READ` 事务里 `pg_export_snapshot()`，把同一快照同时喂给清单读取与 `pg_dump --snapshot`，所以“清单里的行数”描述的就是这份 dump，产出 dump + `manifest_version=1` 清单（恢复点、库名/服务端与客户端版本、schema 版本、每条迁移 SHA-256、每张 `aftercare_` 表行数、dump 字节数与 SHA-256）；`verify` 只查文件不连库；`drill` 建 `aftercare_drill_<时间戳>` 副本、`pg_restore --single-transaction --exit-on-error`、比对迁移摘要/表集合/每张表行数、可选 `--upgrade` 在副本上跑 `migrate()`、最后删副本（`--keep-scratch` 是选择不判失败，清理失败不掩盖原异常）；`retention` 先出计划、`--apply` 才删，保留最新 `keep_last` 份与每 UTC 日一份最多 `keep_daily` 天，永不删空；`reconcile` 只读地列出恢复点无法描述的 Action（全部 `UNKNOWN`，加上 `updated_at` 晚于“恢复点 − 默认 60 s 安全边界”的），不调供应商、不改状态。客户端 major 低于服务端在写任何文件前就失败；密码只经 `PGPASSWORD`，不进命令行与日志。本机实测（临时 PostgreSQL 16.13、客户端 16.13）：dump 111,950 字节、31 张 `aftercare_` 表、迁移 16，`restore 0.703 s`/`verify 0.158 s`/`upgrade 0.151 s`，**RTO 1.012 s**，6 项检查全 ok（报告原文已归档）；`reconcile` 现场列出 7 个 Action（2 个 `UNKNOWN`、5 个晚于恢复点），复跑同一临时库（已被后续全量回归重写过）得到 19 条（2 个 `unknown_outcome` + 17 个 `changed_since_cut`），说明 cut 必须按恢复点取值、不能事后补算（见第 6 节）； 已交付一次性 dump + 清单 + 演练 + 保留计划 + 恢复后核对；**RPO 不由演练发明**，演练只证明该恢复点与该机的 RTO。第二刀（[ADR-0009](decisions/0009-backup-freshness-and-drill-records.md)）把“备份新鲜不新鲜、演练到底做过没有”变成可判定的状态：`drill` 成功与失败都在 dump 旁写 `<name>.drill.json`（恢复点、RTO、逐项检查、失败原因前 400 字符；不含 dump 内容、租户数据或路径），因此目录能区分“演练过但失败”与“从没试过”，而 `drill --json` 仍是可归档的完整报告；新增 `status` 子命令，回答两个不同的问题——最新恢复点是否在 RPO 内、最近一次**成功**演练是否在间隔内（给了 `--drill-interval-seconds` 就隐含要求“存在一次演练”），**预算没有默认值**，未配置只报告不判定并写明 `budgets none stated`，因为未配置不等于已满足；策略问题与完整性问题分开——空目录、dump 缺失、记录不可读无论预算如何都失败或报错，退出码 0/1/2，`--emit-metrics` 走 D-03 的 Metrics 接缝（唯一标签 `component`），仓库不内置调度器；`apply_retention` 连同 `.drill.json` 一起删，名字边界只有 `ops/tooling.py` 一份定义。本机实测（临时 PostgreSQL 16.13）：`create` 后按 24 h 演练间隔查询退出 1（“never recorded in this directory”与“has never been restored”），`drill --upgrade` 退出 0（restore 0.609 s / verify 0.228 s / upgrade 0.310 s，**RTO 1.147 s**，31 张表行数一致）并写出记录，随后双预算查询退出 0。仍缺 WAL 归档与 PITR、对象存储与异地副本、备份加密与密钥托管、按部署目标确定的 RPO/RTO **数值**、把演练接进目标环境的定期执行与告警（cron/systemd/K8s 接线只在文档里，**未在目标环境执行过**）、演练失败后的自动升级路径、多租户选择性恢复 |
...
本轮提交范围为 D-02 第二刀（演练记录与新鲜度判定）：新增 `aftercare_agent/ops/drill_records.py`（`DrillCheck`/`DrillRecord`、`record_version=1`、`OpsModel` 即 `extra=forbid`、`DRILL_SUFFIX=".drill.json"`、`MAX_DETAIL=400`、`drill_record_path()`/`write_drill_record()`/`load_drill_records()`——读不出来是 `OpsError`，不是静默跳过）与 `aftercare_agent/ops/freshness.py`（`ManifestLike` Protocol（只读 property，兼容 frozen 模型）、`StatusBudget`（**无默认值**，非正值抛 `OpsError`）、`Verdict`、`BackupStatus`、`evaluate_status()`、`report_backup_status()`，常量 `BACKUP_METRIC_PREFIX`/`RPO_ENV`/`DRILL_INTERVAL_ENV`）；把 `BACKUP_NAME_PATTERN`/`BackupName`/`validate_backup_name()` 从 `backup.py` 移入 `aftercare_agent/ops/tooling.py`（行为不变，名字边界只留一份定义）；改 `aftercare_agent/ops/backup.py`（新增 `status` 子命令与 `_budget_seconds()`，`drill` 成功失败都写记录，`apply_retention` 同时删除 `.drill.json`）；测试新增 `tests/ops/test_freshness.py`（17 项离线）与 `tests/persistence/test_backup_restore.py` 的 2 项真实 PostgreSQL 用例（共 9 项），`tests/ops/test_backup.py` 的 import 改到 `tooling`；文档新增 `docs/decisions/0009-backup-freshness-and-drill-records.md`（9 条决策、本机实测输出与后果），更新 `docs/operations/backup-restore.md`（新增第 6 节“新鲜度与预算检查（给调度用）”，含 cron 示例与“未在目标环境执行过”声明，原 6/7/8/9 节顺延为 7/8/9/10，文件表加 `.drill.json`）、`docs/development.md`、`docs/tech-stack.md`、`docs/engineering-plan.md`、`ROADMAP.md`、`deploy/compose/README.md` 与本文件。未包含 .venv、缓存、留在 `G:\DevCache\Temp\aftercare-agent` 的演练产物或 EGM 仓库改动。
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 D-02 第二刀（演练记录与新鲜度判定）。交付提交 `59e5945` 已推送到 `origin/main`，CI run [34970075358](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34970075358) 的 `Static checks` 与 `Test with PostgreSQL` 全部为 `success`，在 `postgres:17` 上 `599 passed, 98 warnings in 21.12s`、零跳过（上一版 580 项，本切片新增 19 项 = 离线 17 + 集成 2）。本文件自身的台账提交随后推送，其 CI 结果见第 6 节。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；没有安装或执行过任何 cron/systemd/K8s 调度接线；项目许可证仍待用户确定。
