...
| D-04 | 按瓶颈评估 Broker、暖池、ACP、长期记忆或额外供应商 | 每个新增组件有需要、方案、代价、迁移和验收，未启用项明确保留为候选。已知输入：`Database.transaction()` 每个工作单元建一次连接，本机实测建连 p50 115 ms / 最大 215 ms 而续期事务 p50 0.8 ms；同一套件在本机 71.7 s、在 CI Linux 12.4 s；SSE tail 每轮轮询同样付一次建连，300 ms 尾部窗口在慢建连主机上会漏事件 |
