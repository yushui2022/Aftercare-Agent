...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：把 Aftercare 做成“企业级高并发沙箱审计记忆门控 Agent”，并明确要求本轮“完善沙箱、模型接 DeepSeek（flash）、连接器业务数据参照 Accio CommerceAgentBench 的场景形状”。本轮做三段接线：(1) 模型端接真实 provider（实测模型 ID 是 `deepseek-flash`，“v4.1flash”不存在）；(2) Worker 接真 Harness，把租约、预算、检查点与模型请求串起来；(3) 补整条工具执行面——沙箱 tier/manager、内容寻址 artifact 存储、只读电商连接器与 `SandboxedToolExecutor`，并用 CLI 开关把这条链真正跑通。CommerceAgentBench 只作场景形状参考，不 vendor 其数据、品牌资源或录制响应。不部署、不发布包、不提交、不推送、不连接生产数据库、不做真实退款或外发 |
| 当前任务 | A3-01 模型端接线 + Worker 接真 Harness + C 工具执行面（第三段）。第一段（模型端）：`ResponsesAdapter` 此前按假想 wire 形状写的，实测会拒绝每一个真实响应——顶层白名单只认 7 字段（真实信封 35+）、没有 `reasoning` 输出项分支、`tools` 发的是无 properties 的 schema（模型只能回空参数）。修法：只校验被消费的字段、未知 **output** 项仍 fail-closed；新增 `reasoning` 分支只记“推理发生过”；`tools` 改 `ToolSpec` 带参数 schema；`native_json` 落档前剥掉思维链；新增 `model_adapters/transport.py`（凭据只从环境读，repr/异常/结果都不渲染）。第二段（Worker 端）：`domain/protocol.py` 的 `Checkpoint.pending_tool` 让“崩在工具阶段的模型驱动计划”不必再花一次模型调用找回意图；新增 `runtime/model_harness.py`（`_model_step`/`_tool_step`/`run_model_harness`，预算与上限都落检查点）；`runtime/worker.py` 的 `Harness` 参数从默认实参改成调用时解析，`run_fake_harness` 不再是唯一选择。第三段（工具执行面）：新增 `artifacts.py`（按 SHA-256 寻址、按 tenant/case 分区、原子写、大小上限、读取复核范围与摘要）；`sandbox/manager.py`（`SandboxTierPolicy` 把工作负载类映射到沙箱规格，未知 tier fail closed 不回落；acquire/begin/renew/publish/request_release/confirm_release/release/sweep，容量只在 provider 确认销毁后释放）；`connectors/`（tenant 作用域的电商导出：订单/行项/支付/运单/轨迹/买家留言，校验唯一性与时间顺序；`CommerceConnector` 只读、确定性、答案不含墙钟时间，来源身份由部署给定）；`runtime/sandbox_executor.py`（`ToolExecutor` 实现：租户/工单/订单来自 Run scope 与注入的 case binding，绝不来自工具参数；`request_key` 幂等创建，重放一个已销毁的 key 会申请新 key；产物先写沙箱、再写内容寻址存储并复核摘要；失败路径同样归还容量）；`runtime/wiring.py` 与 `worker_cli` 的 `AFTERCARE_HARNESS_MODEL` 开关（默认仍是确定性 fake；缺数据集、缺来源身份或缺定价都 fail closed） |
...
| 下一项代码候选 | 模型接线剩余：把“模型要的工具有余量”做成持久路由而不是异常，并让 Worker 按 `next_step` 决定 Run 状态（见第 7 节已实测项）；沙箱剩余：真实后端（E2B/K8s）、暖池、出网与短期凭证、独立回收进程与管理 API（C-02 剩余）；连接器剩余：把连接器答案转成 `OrderSnapshot`/`LogisticsObservation` 并入 EGM、支付/退款与供应商回执核对（A3-02、B-02-03）；分层调度：`aftercare_execution_queue.priority` 只被读、从未被写，需要 class 生产者让队列排序、审批阈值与沙箱规格按层生效。D-02 剩余：PITR 演练、对象存储与异地副本、备份加密、RPO/RTO 数值、演练调度接线；D-03 剩余：容器/CI 复跑 sweep、业务队列年龄指标、生产定标；C-03：OTel 导出器；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览 |
| 活跃实现任务 | 新增 `aftercare_agent/artifacts.py`、`sandbox/manager.py`、`connectors/`（`__init__.py`/`dataset.py`/`commerce.py`/`data/commerce-sample.json`）、`runtime/sandbox_executor.py`、`runtime/wiring.py`；改 `sandbox/__init__.py`（导出 manager）、`runtime/worker_cli.py`（`AFTERCARE_HARNESS_MODEL` + 把 harness 透传给 run_once/run_next/run_daemon）、`pyproject.toml`（`connectors/data/*.json` 进 package-data）。测试新增 `tests/test_artifact_store.py`、`tests/test_sandbox_manager.py`、`tests/test_commerce_connector.py`、`tests/test_sandbox_executor.py`、`tests/test_wiring.py`。Broker、暖池、ACP、长期记忆与额外供应商仍未启用 |
...
本轮工作区改动范围（**全部未提交、未推送**）：新增 `aftercare_agent/model_adapters/transport.py`、`aftercare_agent/runtime/model_harness.py`、`aftercare_agent/runtime/sandbox_executor.py`、`aftercare_agent/runtime/wiring.py`、`aftercare_agent/artifacts.py`、`aftercare_agent/sandbox/manager.py`、`aftercare_agent/connectors/`（含 `data/commerce-sample.json`）、`tests/fixtures/model_wire/`、`tests/test_model_wire_compat.py`、`tests/test_model_harness.py`、`tests/test_artifact_store.py`、`tests/test_sandbox_manager.py`、`tests/test_commerce_connector.py`、`tests/test_sandbox_executor.py`、`tests/test_wiring.py`；修改 `aftercare_agent/domain/protocol.py`、`model_adapters/responses.py`、`model_adapters/__init__.py`、`runtime/worker.py`、`runtime/harness.py`、`runtime/worker_cli.py`、`sandbox/__init__.py`、`evals/loop.py`、`tests/test_model_adapters.py`、`tests/evals/test_loop_eval.py`、`pyproject.toml`、`docs/project-status.md`。
...
提交授权：**本轮没有提交或推送授权**。用户 2026-09-16 要求的是“梳理现状、完善沙箱、把模型接上、连接器业务数据参照 Accio CommerceAgentBench 的场景形状”，未要求提交；2026-09-15 的 D-02 推送授权不自动沿用（[AGENTS.md](../AGENTS.md) 工作纪律）。上一轮已推送的提交仍是 `30d3c27`、`130352f`、`e8b8b9b`，本轮的改动只存在于本机工作区。
...
工具执行面之后新增的未决项与风险：(1) **已实测**：模型在工具预算耗尽后仍会继续提议工具，`run_model_harness` 此时抛 `BUDGET_EXHAUSTED`，Run 停在 RUNNING 直到租约过期，而这一轮已经计费的模型调用不会写进任何检查点——重试要再付一次钱。正确做法是把“模型要的工具有余量”做成持久路由（`review` 之类）并让 Worker 按 `next_step` 决定 Run 状态，而不是抛异常；(2) 沙箱 tier 目前只按工具名映射，`aftercare_execution_queue.priority` 仍从未被写入，所以“按层次调度”只在沙箱规格上成立，队列排序与审批阈值还没接；(3) 连接器答案尚未转成 `OrderSnapshot`/`LogisticsObservation` 并入 EGM，`evidence.py` 仍只覆盖退款证据，订单/物流结论暂不进证据账本；(4) `SandboxedToolExecutor` 的 `request_key` 含 worker 身份，换 owner 的恢复会开新沙箱——这是为了让崩溃恢复可行，代价是一次重放的沙箱开销，不是副作用重放。
