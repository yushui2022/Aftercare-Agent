...
| 字段 | 值 |
|---|---|
| 本轮请求范围 | 用户设定目标：按路线图持续把项目推向企业级，并在本轮沿用 2026-09-15 的提交/推送授权。本轮做 D-02 的第三刀，把“恢复点之后的 WAL 还在不在”变成可判定的状态：`aftercare-backup` 新增 `wal` 子命令（归档段是否成链、归档器是否还在推进、归档是否覆盖最新那份 dump；滞后预算没有默认值，`--dsn` 可选），备份清单升到 v2 并记下 dump 覆盖到的 WAL 位置（与恢复点同一快照）；不替部署打开 `archive_mode`、不连接生产数据库、不做真实业务动作、不发布包、不做异地或加密副本 |
| 当前任务 | 流程切片（用户 2026-09-15 反馈“测试的内容怎么那么多…我们只要 20% 的时间是开发”）：把测试与文档成本按硬预算压回产品主线。用 git 逐提交统计出真实账：`aftercare_agent` 13318 行 / `tests` 9999 行 / `docs` 3932 行；历史提交的测试:产品行数比 0.78:1、文档:产品 0.46:1，并且有 13 个只记录 CI 结果的 `docs: record` 提交。据此在 [AGENTS.md](../AGENTS.md) 新增“测试与文档预算”一节：新增测试行数 ≤ 新增产品行数 × 0.5；只为不变量、边界和真实集成写用例；不为错误文案、参数解析分支、配置默认值、纯格式拼装写用例；共用 fixture 只保留一份；每个切片只更新 `docs/project-status.md` 一处文档；不提交只记录 CI 结果的提交。同轮清掉测试里最大的一处复制：`tests/` 此前没有任何 `conftest.py`，同一段 `db` fixture 被逐字节复制 18 份，`database` 4 份、`dsn` 2 份。新增 [tests/conftest.py](../tests/conftest.py) 统一提供 `dsn`/`database`/`db`，从 21 个测试文件删除 21 份重复 fixture（-219 行），需要额外准备数据或严格失败语义的文件（`test_investigations.py`、`test_projection.py`、`test_backup_restore.py`）继续各自定义并遮蔽。本轮不下调任何已有断言强度，也不删除任何覆盖不变量或用例层次的测试。 |
| 当前阶段 | A1-03 DONE；A1-04 最小 Worker/Compose 已有；A2-01 Wait/Inbox/Outbox 与 gap buffer 基础已落地；A2-02 心跳/常驻轮询已落地并完成续期延迟预算修正；B-01 Action Ledger 最小闭环已落地；A3-03-a/b 工作台与 SSE 已完成；D-01-04 撤销判定、D-01-05 授权管理面、D-01-06 工作台访问管理与 D-01-07 租户工单发现已落地；D-04 有界连接池（ADR-0006）、D-03 池指标/容量 sweep（ADR-0007）、D-02 备份/清单/恢复演练（ADR-0008）、演练记录与新鲜度判定（ADR-0009）与 WAL 归档检查（ADR-0010）已落地 |
| 下一项代码候选 | D-02 剩余：按时间点的 PITR 演练本身、对象存储与异地副本、备份加密与密钥托管、按部署目标确定 RPO/RTO 的**数值**、把定期演练接进目标环境的调度与告警、演练失败后的自动升级路径、多租户选择性恢复；D-03 剩余：在容器与 CI 复跑同一条 sweep、把面向业务的队列年龄（Run 队列等待时间）接进观测、在有目标工作负载的部署上定标；C-03：给已接好的 Tracer/Metrics 配导出器与留存；D-01：真实 IdP 演练与权限映射、RLS、租户级授权总览；A3-04：Harness 等待与审批分支纳入评测；B-02-03：供应商回执核对；C-02：真实沙箱后端 |
| 活跃实现任务 | 本轮新增 `aftercare_agent/ops/wal_archive.py`（WAL 段名解析、连续性、归档器状态与覆盖率判定）；`aftercare_agent/ops/backup.py` 新增 `WalLsn` 类型与 `wal` 子命令、`DatabaseSnapshot`/`BackupManifest` 加 `wal_lsn`、`manifest_version` 升到 2；`aftercare_agent/ops/freshness.py` 的 `_age_text()` 公开为 `age_text()`；测试新增 `tests/ops/test_wal_archive.py` 30 项离线与 `tests/persistence/test_wal_archive.py` 3 项真实集群用例，两个演练模块在 `AFTERCARE_REQUIRE_DRILLS=1` 下把跳过当失败；`.github/workflows/ci.yml` 加装 `postgresql-17`、用绝对路径自证并给测试步骤设该变量；文档新增 ADR-0010 并更新运维手册、开发指南、技术栈、执行计划、ROADMAP 与 Compose 说明；Broker、暖池、ACP、长期记忆与额外供应商仍未启用 |
| 本轮外部行为 | 本轮让“恢复点之后的 WAL 是否还在”可以被调度按退出码判定，同时不改动任何数据库的归档配置；不连接生产数据库、不做真实业务动作或生产部署、不上传副本、不安装调度器。本机（临时 PostgreSQL 16.13，含 `archive_mode = on` 的临时集群）重建库后全量 `632 passed, 108 warnings in 77.25s`，离线全量 `509 passed, 123 skipped`，`ruff format --check`/`ruff check`/严格 `mypy`（132 文件）与 `docker compose config --quiet` 通过；远端 CI run [34975613601](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34975613601) 与 [34976339343](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34976339343) 在 `postgres:17` 上均 `632 passed`、零跳过。前端本轮未改也未运行。提交与 CI 结果见第 5、6 节 |
...
本轮提交范围为 D-02 第三刀（WAL 归档检查）：新增 `aftercare_agent/ops/wal_archive.py`（`Segment`/`adjacent()`/`Gap`/`ArchiveInventory`/`ArchiverState`/`ArchiveBudget`/`Coverage`/`ArchiveReport`、`evaluate_archive()`、`report_archive_status()`、`segment_holding()`/`parse_segment_size()`/`parse_lsn()` 与其逆函数，常量 `ARCHIVE_LAG_ENV`/`SEGMENT_PATTERN`/`HISTORY_SUFFIX`/`PARTIAL_SUFFIX`/`LOG_SHIFT`）；改 `aftercare_agent/ops/backup.py`（新增 `WalLsn`、`DatabaseSnapshot.wal_lsn` 在同一 `REPEATABLE READ` 快照里读 `pg_current_wal_lsn()`、`BackupManifest.wal_lsn` 与 `manifest_version: Literal[1, 2] = 2` 的模型校验、`wal` 子命令）；改 `aftercare_agent/ops/freshness.py`（`_age_text()` → 公开 `age_text()`，8 处调用点）；改 `.github/workflows/ci.yml`（加装 `postgresql-17`、用绝对路径自证、测试步骤设 `AFTERCARE_REQUIRE_DRILLS=1`）；测试新增 `tests/ops/test_wal_archive.py`（30 项离线）与 `tests/persistence/test_wal_archive.py`（3 项，自己 `initdb` 一个 `archive_mode = on`、`archive_timeout = 2s` 的临时集群），并让两个演练模块的跳过在该变量下变成失败；文档新增 `docs/decisions/0010-wal-archive-checks.md`（9 条决策、三段实测输出与后果），更新 `docs/operations/backup-restore.md`（新增第 7 节“WAL 归档检查（PITR 的前置条件）”，原 7/8/9/10 节顺延为 8/9/10/11，第 11 节重写“尚未覆盖”）、`docs/development.md`、`docs/tech-stack.md`、`docs/engineering-plan.md`、`docs/persistence.md`、`ROADMAP.md`、`deploy/compose/README.md` 与本文件（并修掉第 3 节里一个混入的 0x07 控制字符）。未包含 .venv、缓存、留在 `G:\DevCache\Temp\aftercare-agent` 的演示产物或 EGM 仓库改动。
...
提交授权：用户在 2026-09-15 明确要求提交并推送到 GitHub；本轮沿用该授权推送 D-02 第三刀。交付提交为 `30d3c27`（WAL 归档检查）、`130352f`（修 CI 自检与临时集群 socket 目录）与 `e8b8b9b`（跳过即失败的闸门），均已推送到 `origin/main`。CI run [34975104046](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34975104046) 虽然结论是 `success`，但工具安装步骤在 `initdb --version` 上以退出码 127 失败、3 项归档用例被跳过（`629 passed, 3 skipped`）——该步当时设了 `continue-on-error`，所以失败没有让作业变红；这正是本切片随后要堵的洞。修复后的 run [34975613601](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34975613601) 与 [34976339343](https://github.com/yushui2022/Aftercare-Agent/actions/runs/34976339343) 在 `postgres:17` 上都是 `632 passed, 98 warnings`、零跳过。本轮没有部署、生产数据或真实业务操作；没有发布 Python 包；没有安装或执行过任何 cron/systemd/K8s 调度接线；项目许可证仍待用户确定。
